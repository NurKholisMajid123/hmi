...
class ProgramKerja {
  ...
  `;
        ...
        // Anggota (6) - bidang, sekretaris, atau bendahara
        else if (roleLevel === 6) {
            ...
            // Jika anggota bidang
            if (bidangId) {
                conditions.push(`p.bidang_id = $${params.length + 1}`);
                params.push(bidangId);
            }

            // Jika anggota sekretaris
            if (isAnggotaSekretaris) {
                conditions.push(`k.nama_kategori = 'Sekretaris'`);
            }

            // Jika anggota bendahara
            if (isAnggotaBendahara) {
                conditions.push(`k.nama_kategori = 'Bendahara'`);
            }
